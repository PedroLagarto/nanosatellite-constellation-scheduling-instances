...
def random_instance(T, jobs, subs=1): 
    orbit_power = np.loadtxt('/home/antunes/my-research/nanosatellite-constellation-scheduling-instances/irradiation/resource.csv')
...
def get_model(instance, coupling=True, recurso=None, new_ineq=False,
    ...
    if new_ineq:
        ...
        for j in J_SUBSET:
            for t in range(T):
                model.addCons(quicksum(phi[j, t_]
